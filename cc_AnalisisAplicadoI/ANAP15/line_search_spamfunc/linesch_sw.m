function [alpha, x, f, grad, fail, nsteps] = ...
           linesch_sw(x0, f0, grad0, d, fgname, c1, c2, prtlevel, dum );
...
%                                                        2011
%                                use with spamfunc       2015
%
%   added dum for unconstrained problems
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 1:nexpand
   ...
   %[fnew, cdum, gradnew] = feval(fgname, xnew, dum, dum ); j-l morales
   %                                                        2015
   %
   % get function and gradient
   %
   [ fnew, cdum ]    = spamfunc( xnew, 0 );
   [ gradnew, Adum ] = spamfunc( xnew, 1 );
   
   gnew = gradnew'*d;
   if fnew > f0 + c1*new*g0 | ((fnew >= fold) & k > 1)   % gone too far
      [alpha, x, f, grad, fail, nsteps] = lszoom(old, new, ...
          fold, fnew, gold, gnew, f0, g0, x0, d, fgname, c1, c2, prtlevel, dum);
      return
   end
   ...
   if gnew >= 0     % gone past minimizer, but new point is better than old
      [alpha, x, f, grad, fail, nsteps] = lszoom(new, old, ... %note reversal
          fnew, fold, gnew, gold, f0, g0, x0, d, fgname, c1, c2, prtlevel, dum);
      return
   end
