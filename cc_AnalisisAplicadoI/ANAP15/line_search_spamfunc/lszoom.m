function [alpha, x, f, grad, fail, nsteps] = lszoom(...
      lo, hi, flo, fhi, glo, ghi, f0, g0, x0, d, fgname, c1, c2, prtlevel, dum)
...
while lo ~= hi & nsteps < 50  % machine precision limits make lo = hi possible
   ...
   %[ftry, cdum, gradtry] = feval(fgname, xtry, dum, dum);  j-l morales
   %                                                        2015
   % get function and gradient
   
   [ ftry, cdum ]    = spamfunc( xtry, 0 );
   [ gradtry, Adum ] = spamfunc( xtry, 1 );  
