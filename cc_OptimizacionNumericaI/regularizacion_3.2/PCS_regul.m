function PCS_regul ( nombre, itermax, tol );
%
...
K = [W0, A0'; A0, speye(m)*0];    % matriz de KKT
...
[L,D,P,S,neg,ran] = ldl(K);    
    ...
    if (ran == (n+m) && neg == m)     %checar si la inercia es correcta para iniciar nuestro m?todo
      inercia_correcta = true;
        ...
        gamma = 10e-4;
        while (k<=10 && not(inercia_correcta))
            k = k+1;             
            K = [W0 + gamma*id, A0'; A0, -delta*speye(m)];    % matriz de KKT
%            fprintf(fout,'\n inercia incorrecta  \n');
